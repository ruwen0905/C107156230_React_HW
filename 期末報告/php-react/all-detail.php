...
$alldetail = mysqli_query($db_connection, "SELECT
    `seq` AS seq,
    `OrderId` AS orderid,
    `ProdId` AS proid,
    `Qty` AS qty,
...
if (mysqli_num_rows($alldetail) > 0) {
    $all_Detail = mysqli_fetch_all($alldetail, MYSQLI_ASSOC);
    // json_encode([],JSON_UNESCAPED_UNICODE) 參數一定要加才會正確顯示中文
    echo json_encode(["success" => 1, "detail" => $all_Detail], JSON_UNESCAPED_UNICODE);
} else {
